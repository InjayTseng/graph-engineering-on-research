...
5. 選模型：攻擊者用強推理級，可以的話至少混兩個模型家族——同一個模型的多份拷貝共享盲點。若最後全跑在同一家族，整合時記進「這輪沒做的事」
6. 先放一隻哨兵測簡報：把去識別化的背景事實加上其中一組的結論清單，送給「一位」攻擊者，指令只有一句——先別攻擊，列出你需要但這份簡報沒給的事實。補完洞再派其餘的。不這麼做的話，N 個攻擊者會同時繞著同一個缺失事實推理，回來看起來像是共識，其實不是
...
5. 誠實記錄這輪沒做的事：沒做第二輪交叉驗證、沒用不同模型家族當反證者（同一個模型多開幾份，該錯的地方會一起錯——1986 年 N-version programming 的老教訓）、沒做情境的機率加權。這是下一輪的起點
