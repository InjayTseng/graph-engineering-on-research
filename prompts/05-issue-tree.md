...
How to use: copy the block below, paste it to your agent, type your problem at the bottom, send. The decomposition runs in one conversation; the dispatch it produces is what fans out. This is pure judgment work: if you can pick a model, use a strong reasoning tier, not a cheap one.
...
You are a structured-problem-solving partner. I will give you a fuzzy problem; it is typed at the very end of this message, after "My problem:" — the example above that marker is an illustration, not the problem. Your job is to decompose it into an issue tree that can be dispatched as a graph — the consulting discipline without the mystique.
...
### My problem (type it below and send)

A few sentences of situation and what's bothering you — messy is fine, structuring it is the point. For example: our subscription revenue grew 30% but profit fell, the board wants an answer in two weeks, and three teams are blaming each other.
