...
想先看它真的跑起來的樣子？以下都是逐字稿（英文），出錯的地方一併留著沒有修掉：

- [00 的實跑紀錄](examples/00-first-graph-run.md)——拿 README 自己舉的例子當主張，三位隔離的懷疑者，九十秒內 3–0 處決
- [02 的實跑紀錄](examples/02-diamond-research-run.md)——31 個 agent 對準這個 repo 自己的前提：「多 agent 真的贏過單一強模型嗎？」死掉的兩條主張都是*反對*多 agent 的那一方，因為驗證者打開論文、發現摘要把結果講過頭了。誠實的答案是：兩個方向都沒人證明出架構層級的效果
- [03 的實跑紀錄](examples/03-adversarial-review-run.md)——被攻擊的文件就是[這個 repo 自己的文章](ARTICLE.md)。26 條凍結的結論、五位隔離的攻擊者，**沒有一條無條件存活**：六條翻案，其中包括文章用來證明「驗證有效」的招牌例子，它本身就是一次誤殺。查證屬實的部分已經改進文章裡了；這份逐字稿就是把它們挖出來的東西
- [04 的實跑紀錄](examples/04-consultant-roundtable-run.md)——五個視角、兩輪，看一個定價決策。四位顧問改口、一位堅持；panel 收斂到以公司為單位計價，但大家採用的那組價格階梯其實出自其中一位，靠錨定擴散而非互相佐證——逐字稿照這樣寫，沒有當成共識報
