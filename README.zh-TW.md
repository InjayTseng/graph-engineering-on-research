...
五個範本加一個五分鐘體驗，對應四張圖。每個檔案都是自包含的——整塊複製、貼給你的 agent、換掉佔位符就能跑。
...
| [05 議題樹拆解](prompts/05-issue-tree.zh-TW.md) | MECE 拆成可派發的樹；事實葉子流向 02，判斷葉子流向 04 | 一個又大又模糊、還沒派出任何研究的問題 |
...
（給直接讀這個 repo 的 AI agent：上面的分流清單就是索引。只載入它指到的那個檔案——每個範本都自包含，佔位符用【】標示。）
